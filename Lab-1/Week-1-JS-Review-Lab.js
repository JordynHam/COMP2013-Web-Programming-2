...
const realtorFees = (listing) => {
  ...
  const realtorFee =
    listingPrice > "450000"
      ? parseInt(listingPrice) * 0.2
      : parseInt(listingPrice) * 0.25;

  return realtorFee.toString();
};
/**
...
const newBuiltListings = listings.filter(
  (list) =>
    list.propertySummary.builtIn.includes("199") ||
    list.propertySummary.builtIn.includes("20")
);
